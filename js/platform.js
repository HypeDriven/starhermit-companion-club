/* Companion Club — StarHermit platform adapter (browser global: window.CCPlatform).
 ...
 * flush; sync status). Offline play is unchanged: no token → localStorage
 * only and zero /api calls. Tokens are kept in memory only — never persisted.
 */
(function (root, factory) {
  var api = factory(root);
  if (typeof module === 'object' && module.exports) module.exports = api;
  else root.CCPlatform = api;
   ...
   * the wrapped save doc. Remote wins on boot (validated through
   * CCStore.loadRaw); saves debounce ~2 s and flush on pagehide/hidden with
   * keepalive; localStorage stays the offline cache. */
  function loadCloud() {
