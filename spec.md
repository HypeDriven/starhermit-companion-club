...
`starhermit.txt` declares `name=Companion Club`, `launch=index.html`, `owner=<id>`, `server=server.js`, `cover=coverart.png`, following the wiki's manifest conventions (https://wiki.starhermit.com/). `server.js` is a plain static file server for the distribution; it implements no game script endpoints. The platform adapter (`js/platform.js`, `CCPlatform`) reads the launch token from the URL fragment `#game_token=<jwt>` (stripped after the read; query forms for local dev), decodes `sub` + `game_scope` (never hard-coded), sends it as `Authorization: Bearer`, and re-mints it every 45 min via `POST /api/v1/games/{slug}/launch-token` (60 s retry). The title screen shows "Playing as <nickname> · sync status" from `GET /api/v1/users/{sub}/profile` (never usernames, never `/api/v1/me`; `Player <id8>` fallback); offline it shows the local line. Finished club days now persist: `js/main.js` records rounds/wins/serves/streak/playMs and sparkle into the `CCStore` save document, unlocks the computable achievements (`first-serve`, `first-win`, `streak-5`, `serves-100`, `sparkle-5000` — the journey/daily/decor/club keys remain data until their modes ship), and `CCStore.save` mirrors the checksummed doc to one zip+base64 cloud slot at `GET/PUT /api/v1/me/cloud-saves/{slug}` (remote wins on boot via `CCStore.loadRaw`, 2 s debounce, `pagehide`/hidden keepalive flush). Without a token the game makes zero `/api` calls and runs fully offline.
